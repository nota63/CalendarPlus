...
def schedule_meeting(request, org_id):
    ...
    try:
        ...
        auto_schedule.scheduled_with.set(users)  # Add matched users
        auto_schedule.save()
        
        return JsonResponse({"success": "Meeting scheduled successfully!", "meeting_id": auto_schedule.id})
    
    except ValidationError as e:
        return JsonResponse({"error": str(e)}, status=400)
    except Exception as e:
        return JsonResponse({"error": "An unexpected error occurred", "details": str(e)}), 500
