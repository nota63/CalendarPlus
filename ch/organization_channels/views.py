...
@login_required
def ban_user(request, org_id, channel_id):
    if request.method == "POST":
        organization = get_object_or_404(Organization, id=org_id)
        channel = get_object_or_404(Channel, id=channel_id, organization=organization)

        # Ensure the request user is the channel creator (admin)
        if channel.created_by != request.user:
            return JsonResponse({'error': 'You are not authorized to ban users in this channel.'}, status=403)

        user_id = request.POST.get('user_id')  # Get user to be banned
        duration = request.POST.get('duration')  # Get duration (e.g., '1_day', '1_week', '1_month', 'permanent', 'specific_date')
        reason = request.POST.get('reason', '')  # Optional reason
        specific_date = request.POST.get('specific_date')  # For custom date (YYYY-MM-DD format)

        user_to_ban = get_object_or_404(User, id=user_id)

        # Determine end_time based on duration
        now = timezone.now()
        if duration == '1_day':
        ...
        elif duration == 'specific_date':
            try:
                end_time = timezone.make_aware(datetime.strptime(specific_date, '%Y-%m-%d'))
            except ValueError:
                return JsonResponse({'error': 'Invalid specific date format. Use YYYY-MM-DD.'}, status=400)
        else:
            return JsonResponse({'error': 'Invalid duration specified.'}, status=400)
        ...
        ActivityChannel.objects.create(
            ...
            organization=organization,
            action_type='LEAVE',
            content=f"Banned user: {user_to_ban.username} for {duration}."
        )

        return JsonResponse({'message': f'User {user_to_ban.username} has been banned.'})
