...
def user_profile(request):
    if request.user.is_authenticated:
        ...
        org_id = profile.organization.id if profile and profile.organization else None  # Ensure org_id is always available

        return {
            "profile": profile,
            "org_id": org_id,  # Make org_id accessible in templates
